...
ShaderProgram setup()
{
    ...
    displayWindow = SDL_CreateWindow("Assignment 5 - Sergey Smirnov", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1280, 720, SDL_WINDOW_OPENGL);
...
int main(int argc, char *argv[])
{
    ...
    while (!done)
    {
        ...
        // left paddle
        float paddle_left_coords[] = { -3.4f, 0.5f, -3.5f, 0.5f, -3.5f, -0.5f, -3.5f, -0.5f, -3.4f, -0.5f, -3.4f, 0.5f };
        ...
        if (game_started)
        {
                     ...
                     (ball.position_x + 0.1f >= paddle_right.left && ball.position_y - 0.1f <= paddle_right.top && ball.position_y + 0.1f >= paddle_right.bottom))
            {
                ...
                ball.move();
                ball_matrix.Translate((ball.speed * ball.direction_x), (ball.speed * ball.direction_y), 0.0f);
                if (ball.position_x - 0.1f <= paddle_left.right)
                    Mix_PlayChannel(-1, paddle_left_hit, 0);
                else
                    Mix_PlayChannel(-1, paddle_right_hit, 0);
            }
    ...
    Mix_FreeChunk(paddle_left_hit);
    Mix_FreeChunk(paddle_right_hit);
    Mix_FreeMusic(music);
